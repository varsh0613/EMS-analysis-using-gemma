...
OUT_DIR = r"C:\Users\SAHARA\OneDrive\Desktop\uni\gemma\risk_score_fixed\outputs"
...
def run_pipeline(input_csv: str, out_dir: str):
    ...
    cat_cols = ["primary_impression", "protocol_used"]
    X_cats_train = df_train[cat_cols].fillna("").astype(str)
    ...
    for col_idx, categories in enumerate(ohe.categories_):
        ...
        for cat in categories:
            safe = safe_name(cat)
            feature_names.append(f"{prefix}__{safe}")

    feature_names = make_unique(feature_names)
    X_train_df = pd.DataFrame(X_ohe_train, columns=feature_names, index=df_train.index)

    # Add numeric age
    patient_age_train = pd.to_numeric(df_train.get("Patient_Age", pd.Series([np.nan]*len(df_train))), errors="coerce").fillna(-1)
    X_train_df["patient_age"] = patient_age_train.values
    ...
    X_cats_test = df_test[cat_cols].fillna("").astype(str)
    X_ohe_test = ohe.transform(X_cats_test)
    X_test_df = pd.DataFrame(X_ohe_test, columns=feature_names, index=df_test.index)

    patient_age_test = pd.to_numeric(df_test.get("Patient_Age", pd.Series([np.nan]*len(df_test))), errors="coerce").fillna(-1)
    X_test_df["patient_age"] = patient_age_test.values
